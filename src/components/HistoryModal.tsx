        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
            ...
            <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[2rem] shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] w-full max-w-md h-[80vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <div className="p-6 border-b border-jaap-primary/20 flex items-center justify-between">
                    ...
                    <button
                        onClick={onClose}
                        className="text-foreground/70 hover:text-foreground transition-colors p-1 hover:bg-jaap-primary/10 rounded-full"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {history.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-foreground/60 text-center p-8">
                        ...
                        history.slice().reverse().map((session) => (
                            ...
                            >
                                <div>
                                    {/* Mantra badge */}
                                    {session.mantraName && (
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-jaap-primary mb-1 flex items-center gap-1">
                ...
                <div className="p-4 border-t border-jaap-primary/20 bg-jaap-primary/5">
                    <button
                        ...
                        className="w-full py-3 rounded-xl flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-foreground/60 hover:text-foreground hover:bg-jaap-primary/10 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
